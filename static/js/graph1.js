 ...
 *  1. Sends ALL selected documents (POST) so multi-doc graph works correctly.
 *  2. Edge tooltip shows which PDF(s) each relation was extracted from.
 *  3. Node tooltip also shows originating PDF(s).
 *  4. D3 lazy-load preserved exactly as in original (fixes "d3 is not defined").
 */
...
const KnowledgeGraph = (() => {
  ...
  function _injectModal() {
    ...
    style.textContent = `
      ...
      #kgPanel {
        ...
        border-radius: 20px;
        box-shadow: 0 30px 80px rgba(0,0,0,.7), 0 0 60px rgba(167,139,250,.08);
      ...
      #kgHeader h3 {
        ...
        color: #e2d9f3; letter-spacing: .02em;
        flex: 1;
      }
      #kgHeader .kg-icon { color: #a78bfa; font-size: 1.1rem; }
      ...
      .kg-legend-item:hover { background: rgba(255,255,255,.07); }
      ...
      .kg-zoom-btn {
        ...
        border-radius: 8px; color: #e2d9f3;
        font-size: .9rem; cursor: pointer;
        display: flex; align-items: center; justify-content: center;
        transition: background .2s;
      }
      .kg-zoom-btn:hover { background: rgba(167,139,250,.25); }
      ...
      #kgTooltip {
        ...
        font-size: .78rem; color: #e2d9f3;
        pointer-events: none; opacity: 0;
        transition: opacity .15s;
        max-width: 280px; box-shadow: 0 8px 24px rgba(0,0,0,.5);
        line-height: 1.5;
      }
      #kgTooltip.visible { opacity: 1; }
      #kgTooltip .kg-tt-label { font-weight: 600; color: #a78bfa; margin-bottom: 4px; }
      #kgTooltip .kg-tt-type  { font-size: .7rem; color: #6b7280; text-transform: uppercase; letter-spacing: .06em; }
      #kgTooltip .kg-tt-rel   { font-style: italic; color: #67e8f9; margin-top: 4px; }
      #kgTooltip .kg-tt-conn  { color: #9ca3af; margin-top: 4px; }
      #kgTooltip .kg-tt-src   {
        margin-top: 7px; padding-top: 7px;
        border-top: 1px solid rgba(167,139,250,.2);
        font-size: .7rem; color: #6b7280;
      }
      #kgTooltip .kg-tt-src b { color: #fbbf24; }
      #kgTooltip .kg-tt-src span { display: block; color: #9ca3af; margin-top: 2px; }
      ...
      #kgEmpty i { font-size: 2.5rem; color: rgba(167,139,250,.3); }
      ...
      #kgCloseBtn:hover { color: #e2d9f3; }

      .kg-node { cursor: pointer; }
      .kg-node circle { transition: r .15s, filter .15s; }
      .kg-node:hover circle { filter: brightness(1.3); }
      .kg-link {
        stroke-opacity: .45;
      ...
      .kg-label {
        font-size: 10px; fill: #c4b5fd;
        pointer-events: none;
        paint-order: stroke;
        stroke: rgba(5,5,15,.9);
        stroke-width: 3px;
        font-family: 'Sora', sans-serif;
      }
      .kg-edge-label {
        font-size: 8.5px; fill: #67e8f9;
        pointer-events: none;
        paint-order: stroke;
        stroke: rgba(5,5,15,.9);
        stroke-width: 3px;
  ...
  function _buildGraph(nodes, edges, svgWrap) {
    svgWrap.innerHTML = "";          // clear previous render
    ...
    function _srcHtml(sources) {
      ...
      const names = sources.join("<br>");
      return `<div class="kg-tt-src"><b>&#128196; PDF source${sources.length > 1 ? "s" : ""}:</b><span>${names}</span></div>`;
    }
  ...
  async function open() {
    ...
    // Ensure D3 is loaded before doing anything with it
    ...
    // ── Gather selected files and POST to backend ─────────────────────────
    const files     = _getSelectedFiles();
    const sessionId = (typeof session_id !== "undefined" && session_id) ? session_id : "kg-session";

    try {
      // Use POST so we can send the full file list safely (no URL length issues)
      const res = await fetch("/knowledge_graph", {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({
          session_id: sessionId,
          files:      files,        // all checked docs — empty = all user docs
        }),
      });
      ...
      _buildGraph(
        JSON.parse(JSON.stringify(allNodes)),  // deep copy for simulation mutation
  ...
  // Pre-load D3 in background on page load so it's ready when user clicks the button
  window.addEventListener("load", () => {
    if (typeof d3 === "undefined") _loadD3().catch(() => {});
    _injectModal();
  });

  return { open, close, toggleType, zoom, resetZoom };
